...
export BUILD_ARGS="--rm=false --build-arg TAG=${CIRCLE_TAG} --build-arg SHA1=${CIRCLE_SHA1} --build-arg DATE=$(date +%F-%T) --build-arg GITHUB_ACCESS_TOKEN=${GITHUB_ACCESS_TOKEN}"
echo  "BUILD_ARGS: $BUILD_ARGS"

# docker build -t "21zoo/redis_exporter:$CIRCLE_TAG" $BUILD_ARGS .
# docker push     "21zoo/redis_exporter:$CIRCLE_TAG"

docker build -t "redis_exporter:latest" $BUILD_ARGS .
